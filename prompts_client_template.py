...
# Use this version for CLIENT-FACING deployments. Before going live for a client:
#   • SESSION_INSTRUCTION opens with a VERBATIM AI + call-recording disclosure.
#     California is an ALL-PARTY consent state — keep the "recorded" wording ONLY
#     if that deployment actually records audio; remove it if it does not.
#   • Replace "Aperture Automations" in the disclosure with the client's business
#     name, and give the client its own persona / "WHO YOU SERVE" block instead of
#     Lloyd's.
# (Lloyd's personal assistant version lives in prompts.py.)
